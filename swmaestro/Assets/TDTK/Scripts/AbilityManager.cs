...
namespace TDTK {
	...
	public class AbilityManager : MonoBehaviour {
				...
				if(Input.touchCount>=1){
					...
					if(mainCam!=null){
						...
						if(Physics.Raycast(ray, out hit, Mathf.Infinity, mask)){
							...
							if(ability.singleUnitTargeting){
								targetUnit=hit.transform.GetComponent<Unit>();
								if(targetUnit==null) validTarget=false;
							}
							
							if(Input.touches[0].phase!=TouchPhase.Ended) return;
							if(validTarget){
								ActivateAbility(ability, hit.point, targetUnit);
		...
		public static int GetAbilityIndex(Ability ab){
			for(int i=0; i<instance.abilityList.Count; i++){
				if(ab==instance.abilityList[i]) return i;
			}
			return -1;
		}
	}
