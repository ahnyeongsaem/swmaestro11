...
namespace TDTK {
	...
	public class UnitTowerEditorWindow : UnitEditorWindow {
		...
		Vector2 DrawUnitConfigurator(float startX, float startY, List<UnitTower> towerList){
			...
			if(GUI.Button(new Rect(startX+215, startY, 48, 15), "Select")){ if(tower.nextLevelTowerList[0]!=null) SelectTower(ID-1); }
			...
			if(tower.nextLevelTowerList[0]!=null){
				...
				if(GUI.Button(new Rect(startX+215, startY, 48, 15), "Select")){ if(tower.nextLevelTowerList[1]!=null) SelectTower(ID-1); }
