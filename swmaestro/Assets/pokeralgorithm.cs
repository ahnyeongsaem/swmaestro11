	...
	public class pokeralgorithm : MonoBehaviour {
		...
		int pokercheck()
		{
			...
			//flash
			        ...
			        card[0].type==card[4].type)
			{
				PerkManager.PurchasePerk(5);
				return 5;
			}
			//st
			else if(card [1].number == card [2].number - 1 &&
			        card [2].number == card [3].number - 1 &&
			        card [3].number == card [4].number - 1 &&
			        (card [0].number == card [1].number - 1 
			 || (card [0].number == 1 && card [4].number == 13) ) )
			{
				PerkManager.PurchasePerk(4);
				return 4;
			}
			//triple
			else if( (card[0].number == card[1].number && card[1].number==card[2].number) ||
			        (card[1].number==card[2].number && card[2].number==card[3].number) ||
			        (card[2].number==card[3].number && card[3].number==card[4].number)
			        )
			{
				PerkManager.PurchasePerk(3);
				return 3;
			}
			//two pair
			else if( (card[0].number==card[1].number && card[2].number==card[3].number) ||
					(card[0].number==card[1].number && card[3].number==card[4].number) ||
					(card[1].number==card[2].number && card[3].number==card[4].number) )
			{
				PerkManager.PurchasePerk(2);
				return 2;
			}
			//one pair
			else if( card[0].number==card[1].number || card[1].number==card[2].number ||
			...
			{
				PerkManager.PurchasePerk(1);
				return 1;
			}
			else
			{
				PerkManager.PurchasePerk(0);
				return 0;
			}
			return -1;
		}
